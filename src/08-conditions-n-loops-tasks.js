...
function doRectanglesOverlap(rect1, rect2) {
  const y2Rect1 = rect1.height + rect1.top;
  const x4Rect1 = rect1.width + rect1.left;
  if (rect2.top > y2Rect1 || rect2.left > x4Rect1) {
    return false;
  }
  return true;
}
...
function getMatrixProduct(m1, m2) {
  ...
  for (let k = 0; k < m2[0].length; k += 1) {
    for (let i = 0; i < m1.length; i += 1) {
      let sum = 0;
      for (let j = 0; j < m2.length; j += 1) sum += m1[i][j] * m2[j][k];
      result[i][k] = sum;
    }
  }
  return result;
...
function evaluateTicTacToePosition(position) {
  if (position[0][0] === position[1][1]
    && position[0][0] === position[2][2] && position[0][0] !== undefined) {
    return position[0][0];
  }
  if (position[0][2] === position[1][1]
    && position[0][2] === position[2][0] && position[0][2] !== undefined) {
    return position[0][2];
  }
  for (let i = 0; i < 3; i += 1) {
